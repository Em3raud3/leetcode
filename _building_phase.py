...
def check_adjacent(row, col, table, visitation, size, id):
    ...
    try:
        if table[row][col + 1] and table[row][col + 1] == 1:
            table[row][col + 1] = id
            size += 1
            size = check_adjacent(row, col + 1, table, visitation, size, id)
    except Exception:
        pass
    ...
    try:
        if table[row + 1][col] and table[row + 1][col] == 1:
            table[row + 1][col] = id
            size += 1
            size = check_adjacent(row + 1, col, table, visitation, size, id)
    except Exception:
        pass
...
for i in range(n):
    for j in range(m):
        if grid[i][j] == 1:
            islandID += 1
            grid[i][j] = islandID
            localsize = check_adjacent(i, j, grid, visited, 1, islandID)
